...
if __name__ == "__main__":
	...
	q_des = q[7:].copy()
	t = 0.
	o,x0,u0=[0.,0.],[0.,0.],[0.,0.,0.,0.]
	u=[0,0,0,0]
	for i in range(horizon_length):

		q, dq = bipd.get_state_update_pinocchio() # x
		com = se3.centerOfMass(bipd.pinocchio_robot.model,bipd.pinocchio_robot.data, q) # This is what I want
		...
		for j in range(len(x)):
			...
			pid.SetPoint=float(x0[j])
			pid.update(float(x[j]), t)
			o[j]=pid.output
		lu=o
		ru=o
		u=lu+ru
